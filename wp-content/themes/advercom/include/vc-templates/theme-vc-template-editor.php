...
class Pao_Vc_Templates_Editor {
    ...
    public function addTemplatesTab( $data ) {
        $newCategory = array(
            'category'        => 'pao_templates',
            'category_name'   => esc_html__( 'Pao Templates', 'pao' ),
            'category_weight' => 1,
    ...
    protected function get_template_categories() {
        ...
        $categories = array(
            'all'         => esc_html__( 'All', 'pao' ),
            'banner'      => esc_html__( 'Banners', 'pao' ),
            'about'      => esc_html__( 'About', 'pao' ),
            'skills'    => esc_html__( 'Skills', 'pao' ),
            'faq'         => esc_html__( 'FAQ', 'pao' ),
            'map'         => esc_html__( 'Maps', 'pao' ),
            'contacts'    => esc_html__( 'Contacts', 'pao' ),
            'pricing'     => esc_html__( 'Pricing', 'pao' ),
            'service'     => esc_html__( 'Services', 'pao' ),
            'shop'        => esc_html__( 'Shop', 'pao' ),
            'info-block'  => esc_html__( 'Info Block', 'pao' ),
            'team'        => esc_html__( 'Team', 'pao' ),
            'testimonial' => esc_html__( 'Testimonials', 'pao' ),
            'call-to-action'        => esc_html__( 'Call to action', 'pao' ),
            'video'        => esc_html__( 'Video', 'pao' ),
            'coming-soon'        => esc_html__( 'Coming Soon', 'pao' ),
            'instagram'        => esc_html__( 'Instagram', 'pao' ),
            'headings'        => esc_html__( 'Headings', 'pao' ),
            'portfolio'        => esc_html__( 'Portfolio', 'pao' ),
            'line-of-images'        => esc_html__( 'Line of images', 'pao' ),
            'posts'        => esc_html__( 'Posts', 'pao' ),
            'animation'        => esc_html__( 'Animation', 'pao' ),
        );
    ...
    public function renderTemplateBlock( $category ) {
        ...
        if ( 'pao_templates' === $category['category'] ) {

            $category['output'] = '<div class="vc_col-md-2 pao-sorting-container">';
            $category['output'] .= $this->get_template_categories();
            $category['output'] .= '</div>';


            $category['output'] .= '
			<div class="vc_column vc_col-sm-12 pao-templates-container">
    ...
    function renderTemplateWindow( $template_name, $template_data ) {
        if ( 'pao_templates' === $template_data['type'] ) {
            return $this->renderTemplateWindowPaoTemplates( $template_name, $template_data );
        }
     ...
     * @return string
     */
    public function renderTemplateWindowPaoTemplates( $template_name, $template_data ) {
        ob_start();
        ...
        $preview_template_title = esc_attr( 'Preview template', 'pao' );
        $add_template_title = esc_attr( 'Add template', 'pao' );
...
HTML;
    ...
    public function renderBackendTemplate() {
        ...
        if ( ! isset( $template_id, $template_type ) || '' === $template_id || '' === $template_type ) {
            die( 'Error: Vc_Pao_Templates::renderBackendTemplate:1' );
        }
    ...
    public function getAllTemplates() {
        ...
        $data = array();
        $pao_templates = $this->getTemplates();
        $category_templates = array();
        foreach ( $pao_templates as $template_id => $template_data ) {
            $category_templates[] = array(
    ...
    public function loadDefaultTemplates() {
        ...
        if ( ! is_array( $this->pao_templates ) ) {
            $this->pao_templates = $this->allTemplates();
        }

        return $this->pao_templates;
    }
    ...
    public function getDefaultTemplate( $template_index ) {
        ...
        if ( ! is_numeric( $template_index ) || ! is_array( $this->pao_templates ) || ! isset( $this->pao_templates[ $template_index ] ) ) {
            return false;
        }

        return $this->pao_templates[ $template_index ];
    }
    ...
    public function addDefaultTemplates( $data ) {
        if ( is_array( $data ) && ! empty( $data ) && isset( $data['name'], $data['content'] ) ) {
            if ( ! is_array( $this->pao_templates ) ) {
                $this->pao_templates = array();
            }
            $this->pao_templates[] = $data;
    ...
    public function renderTemplateListItem( $template ) {
        $name = isset( $template['name'] ) ? esc_html( $template['name'] ) : esc_html( __( 'No title', 'pao' ) );
...
HTML;
        $output .= '<div class="pao-template-preview">';
        if ( $new ) { $output .= '<span class="pao-badge-new">New</span>'; }
        ...
        $output .= '<span class="pao-template-categories">' . esc_html( $template_sort_name ) . '</span>';
        $output .= <<<HTML
						</div>
