;(function ($, window, document, undefined) {
    ...
    function topBannerHeight() {
        ...
        if($('.right-menu.full #topmenu').length){
            $('.right-menu.full #topmenu').height(windowH - footerH);
        }
    ...
    function onYouTubeIframeAPIReadyPao() {
        if (_ismobile) {
        ...
        if (window.products_load) {
            ...
            $('.product-tabs-wrapper .filters ul li').on('click', function (e) {
                ...
                var cats = $(this).attr('data-filter'),
                    ...
                    autoplay = $(this).closest('.product-tabs-wrapper').find('.pao-product-filter').attr('data-autoplay'),
                    speed = $(this).closest('.product-tabs-wrapper').find('.pao-product-filter').attr('data-speed'),
                    ...
                    loop = $(this).closest('.product-tabs-wrapper').find('.pao-product-filter').attr('data-loop');
                ...
                $.ajax({
                    ...
                    type: "post",
                    data: ({
                        action: 'pao_products_slider_load',
                        cats: cats,
                        order: order,
                        orderby: orderby,
                        count: count,
        ...
        if (window.portfolio_load) {
            ...
            $('.portfolio-tabs-wrapper .filters ul li').on('click', function (e) {
                ...
                $.ajax({
                    ...
                    type: "post",
                    data: ({
                        action: 'pao_portfolio_slider_load',
                        cats: cats,
                        order: order,
                        orderby: orderby,
                        count: count,
    ...
    function calcPaddingBlog() {
        if($('.blog.center').length && $('body.blog .header_trans-fixed')) {
            var headerHeight = $('.header_trans-fixed').outerHeight();
            $('.blog.center').css('padding-top', headerHeight + 'px');
        }


        if($('.blog.metro, .blog.masonry').length && $('body.blog .header_trans-fixed')) {
            var headerHeight = $('.header_trans-fixed').outerHeight();
            var dataTop = ($(window).width() > 767) ? 30 : 20;
            $('.blog.metro, .blog.masonry').css('padding-top', headerHeight + dataTop + 'px');
        }
    }
    ...
    $(window).on('resize scroll', function () {
        if ($('.headings-wrap').length && $('.headings-wrap').hasClass('load-fade')) {
            $('.title, .subtitle, .but-wrap, .title-bg, .but-wrap, .text').each(function () {
    ...
    // header social
    $('.pao-top-social .social-icon').on("click", function() {
        var thisItem = $(this);
        var thisItemParent = thisItem.parent('.pao-top-social');
    ...
    /* Copyright */
    if ($('.pao_copyright_overlay').length) {
        $(document).on('contextmenu',function(event){
            if($('.pao_copyright_overlay').hasClass('copy')){
                event.preventDefault();
            }else if(event.target.tagName != 'A'){
                event.preventDefault();
            }
            $('.pao_copyright_overlay').addClass('active');
        }).on('click', function(){
            $('.pao_copyright_overlay').removeClass('active').removeAttr('style');
        });
    }
    ...
    function load_more_blog_posts() {
        ...
        if (window.load_more_blog_posts) {
            ...
            //button click
            if($('.blog .js-load-more').length && pageNum >= max){
                $('.blog .js-load-more').hide('fast');
            }
