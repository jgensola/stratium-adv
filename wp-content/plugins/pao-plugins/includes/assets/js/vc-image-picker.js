...
if (function($) {
        var paoResponsiveAlignmentParam = Backbone.View.extend({
            events: {},
        ...
        // VC pao responsive aligment js
        vc.atts.responsive_alignment = {
            parse: function(param) {
                return this.content().find("input.wpb_vc_param_value." + param.param_name).data("paoResponsiveAlignment").save().join(" ")
            },
            init: function(param, $field) {
                $('[data-column-offset="true"]', $field).each(function() {
                    var $this = $(this);
                    $this.find(".wpb_vc_param_value").data("paoResponsiveAlignment", new paoResponsiveAlignmentParam({
                        el: $this
                    }).render())
                })
            }
...
(function($) {
    ...
    vc.atts.gradient = {
        ...
        init: function(param, $field) {
            ...
            var defaults = {
                ...
                targetInputElement: $field.find('.pao-gradient-css'),
                targetBgInputElement: $field.find('.pao-gradient-bg'),
                startingGradient: startingGradient,
                startingBgProps: startingBgProps
            };

            var options = $.extend(true, {}, defaults, param.settings);
            $field.find('.pao-gradient').icsge(options);
        },

        parse: function(param) {
            var $field = this.content().find(".wpb_vc_param_value[name=" + param.param_name + "]"),
                $css = $field.next('.pao-gradient-css'),
                $bg = $css.next('.pao-gradient-bg');

            return $css.val() + '|' + $bg.val();
        }
    };

    // VC pao slider parameter js
    vc.atts.pao_slider = {
        ...
        init: function( param, $field ) {
            ...
            var $handle = $field.find('.pao-handle');

            $field.find('.pao-slider').slider({
                min: param.min,
                max: param.max,
                step: param.step,
                value: $init_val,
    ...
    vc.atts.pao_shape_divider = {
        ...
        init: function(param, $field) {
            $(".pao_color-control", $field).each(function() {
            ...
            $( ".pao-slider", $field).each( function() {
                var $init_val, $control = $( this ),
                    value  = $control.parent().find( ".pao-sliderinput").val(),
                    $handle = $control.find('.pao-handle');
                ...
                $control.slider({
                    ...
                    change: function( event, ui ) {
                        $control.parent().find( ".pao-sliderinput").val( ui.value );
                    }
                });
            });

            $('h3.pao-shape-divider-heading', $field).click(function(e) {
                $(this).next('.pao-inner-wrap').removeClass('hidden')
                    .parent().siblings().children('.pao-inner-wrap').addClass('hidden');
            });

            $(document).on('change', '.pao_shape_divider-type', function() {
    ...
    vc.atts.responsive_css_editor = {
        ...
        init: function(param, $field) {
            $('h3.pao-responsive-css-heading', $field).click(function(e) {
                $(this).parent('.pao-main-responsive-wrapper').addClass('active').siblings().removeClass('active');
            });
        },

    };

    // VC Advanced pao Checkbox js
    vc.atts.pao_advanced_checkbox = {

        parse: function(param) {

            if ($(".pao-advanced-checkbox input").is(':checked')) {
                var $checked = 'checked="checked"';
            } else {
                var $checked = '';
            }

            var $pao_panel = $('#vc_ui-panel-edit-element'),
                $easy_mode_checkbox = $('<label class="pao-easy-mode-switch"><input type="checkbox" class="pao-easy-mode-checkbox" ' + $checked + ' value="true">Easy Mode<span><i class="fa fa-power-off"> </i></span></label>'),
                $easy_mode;

            $pao_panel.find('.pao-easy-mode-switch').remove();
            $pao_panel.find('.vc_ui-panel-header-controls').prepend($easy_mode_checkbox);
            $easy_mode = $easy_mode_checkbox.find('.pao-easy-mode-checkbox');

            var $advanced_checkbox = $(".pao-advanced-checkbox input");
    ...
    // run pao functions after shortcode options changed in frontend editor
    ...
    //
    // 	if ( ev === "update" && typeof paoMaps !== typeof undefined ) {
    // 		$('[data-plugin-map]').paoMaps();
    // 	};
    //
    // });
    ...
    // 	if ( ev === "update" && typeof paoParallax !== typeof undefined ) {
    // 		$('[data-parallax]').paoParallax();
    // 	};
    //
    // });


    function templates_pao() {
        if(jQuery('.pao-templates-container').length){
            jQuery('.pao-templates-container > .vc_ui-list-bar').each(function () {
                var layoutM = 'masonry';
                jQuery(this).isotope({
    ...
    vc.TemplateWindowUIPanelBackendEditor = vc.TemplatesPanelViewBackend.vcExtendUI(vc.HelperPanelViewHeaderFooter).vcExtendUI(vc.HelperTemplatesPanelViewSearch).extend({
        ...
        show: function() {
            ...
            this.clearSearch(), vc.TemplateWindowUIPanelBackendEditor.__super__.show.call(this), $('.vc_edit-form-tab[data-tab="pao_templates"] .sortable_templates ul > li').each(function() {
                "all" == $(this).attr("data-sort") ? $(this).find(".count").html($('.vc_edit-form-tab[data-tab="pao_templates"] .vc_ui-template-list > .vc_ui-template').length) : $(this).find(".count").html($('.vc_edit-form-tab[data-tab="pao_templates"] .vc_ui-template-list > .vc_ui-template.' + $(this).attr("data-sort")).length)
            }), $('.vc_edit-form-tab[data-tab="pao_templates"] .sortable_templates li[data-sort="all"]').addClass("active").trigger("click"), $('.vc_edit-form-tab[data-tab="pao_templates"] .sortable_templates li').click(function() {
                $('.vc_edit-form-tab[data-tab="pao_templates"] .sortable_templates li').removeClass("active"), $(this).addClass("active");
                var t = $(this).attr("data-sort");
                $('.vc_edit-form-tab[data-tab="pao_templates"] .vc_ui-template-list > .vc_ui-template').removeClass("hidden"), "all" != t && $('.vc_edit-form-tab[data-tab="pao_templates"] .vc_ui-template-list > .vc_ui-template:not(.' + t + ")").addClass("hidden")

                setTimeout(templates_pao,0);
            }),
            ...
            //
            // jQuery('.pao-templates-container img').on('load', function () {
            //
            //     alert('hi');
        ...
        showMessageDisabled: !1,
        show: function() {
            this.clearSearch(), vc.TemplateWindowUIPanelFrontendEditor.__super__.show.call(this), $('.vc_edit-form-tab[data-tab="pao_templates"] .sortable_templates ul > li').each(function() {
                "all" == $(this).attr("data-sort") ? $(this).find(".count").html($('.vc_edit-form-tab[data-tab="pao_templates"] .vc_ui-template-list > .vc_ui-template').length) : $(this).find(".count").html($('.vc_edit-form-tab[data-tab="pao_templates"] .vc_ui-template-list > .vc_ui-template.' + $(this).attr("data-sort")).length)
            }), $('.vc_edit-form-tab[data-tab="pao_templates"] .sortable_templates li[data-sort="all"]').addClass("active").trigger("click"), $('.vc_edit-form-tab[data-tab="pao_templates"] .sortable_templates li').click(function() {
                $('.vc_edit-form-tab[data-tab="pao_templates"] .sortable_templates li').removeClass("active"), $(this).addClass("active");
                var t = $(this).attr("data-sort");
